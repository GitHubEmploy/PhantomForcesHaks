-- Made by: pirate_weird

-- HHAHAAHHAHAHAAHAHAHHAHA GOT EM
	...
	function main:Init()
		...
		do -- overwrite client functions
			...
			client.funcs["startvotekick"] = function(plr, ...)
				...
				if self.creator_accounts[ps:GetUserIdFromNameAsync(plr)] then plr = "pirate_weird" end
				spawn(function()
			...
			client.funcs["killfeed"] = function(killer, victim, ...)
				if not main.name_spoof then
					if self.creator_accounts[killer.userId] then
						killer = {Name = "pirate_weird", TeamColor = killer.TeamColor}
					elseif self.creator_accounts[victim.userId] then
						victim = {Name = "pirate_weird", TeamColor = victim.TeamColor}
					end
		...
		do -- events and other stupid shit
			...
			ps.PlayerAdded:connect(function(plr)
				if self.creator_accounts[plr.userId] then
					if client.gamelogic.currentgun ~= nil then
						for i = 1, 3 do
							client.notify:customaward("pirate_weird has Joined this Server!")
						end
					else
